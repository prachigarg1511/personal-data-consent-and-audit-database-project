-- SQLTools-friendly version (no DELIMITER statements)
-- Recommended execution: run in chunks (schema -> views -> triggers -> procedures -> seed -> procedure calls)
...
-- Triggers (run each CREATE TRIGGER separately if your runner complains)
DROP TRIGGER IF EXISTS trg_consent_uuid;
...
DROP TRIGGER IF EXISTS trg_deletion_uuid;
...
DROP TRIGGER IF EXISTS trg_consent_history_create;
...
DROP TRIGGER IF EXISTS trg_consent_set_revoked_at;
...
DROP TRIGGER IF EXISTS trg_consent_history_update;
...
-- Stored procedures (run each CREATE PROCEDURE separately if your runner complains)
DROP PROCEDURE IF EXISTS sp_expire_consents;
...
DROP PROCEDURE IF EXISTS sp_grant_consent;
...
DROP PROCEDURE IF EXISTS sp_add_scope_field;
...
DROP PROCEDURE IF EXISTS sp_create_deletion_request;
...
DROP PROCEDURE IF EXISTS sp_complete_deletion_request;
...
-- Create initial consents (RUN THESE ONLY AFTER procedures are created successfully)
